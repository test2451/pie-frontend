...
const UserName: React.FC = () => {
  ...
  const { teamId, tokenId, userName, actions, minimumPieRequired, allowance } = useProfileCreation()
  ...
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumPieRequired={minimumPieRequired}
      allowance={allowance}
    />,
  )
    ...
    <>
      ...
      {!hasMinimumPieRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${minimumCakeBalance} PIE is required`)}
        </Text>
      )}
    </>
  )
