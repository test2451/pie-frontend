...
import { Button, InjectedModalProps, Skeleton, Text } from '@pieswap-libs/uikit'
...
import { usePieRabbits, useProfile as useProfileContract } from 'hooks/useContract'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const {
    ...
    onApprove: () => {
      return pieRabbitsContract.methods.approve(getPieProfileAddress(), tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
        return profileContract.methods.reactivateProfile(getPieRabbitsAddress(), tokenId).send({ from: account })
      }

      return profileContract.methods.updateProfile(getPieRabbitsAddress(), tokenId).send({ from: account })
    },
    onSuccess: async () => {
      // Re-fetch profile
    ...
    <>
      ...
      {!isLoading && walletNfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(999, 'Make sure you have a Pie Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
